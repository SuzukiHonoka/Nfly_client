...
type Methods interface {
	...
	Register(usr string, pass string)
	Delete(usr string)
	Feeds()
	Collect()
	Push()
...
func (x *API) Login(usr string, pass string) bool {
	data, error := x.Request.PostForm(apiURL["login"].String(),url.Values{
		"email":[]string{usr},
...
func (x *API) Register(usr string, pass string) bool {
	data, error := x.Request.PostForm(apiURL["register"].String(),url.Values{
		"email":[]string{usr},
		"password":[]string{pass},
	})
	if error != nil {
...
func (x *API) Delete(usr string) bool{
	...
	data, error := x.Request.Delete(target.String())
	if error != nil {
		log.Fatal(error)
	}
	return data.Status
